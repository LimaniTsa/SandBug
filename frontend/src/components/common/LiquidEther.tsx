  ...
  useEffect(() => {
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(...args: any[]) {
        ...
        const centerX = Math.min(Math.max(Mouse.coords.x, -1 + cs * cellScale.x + cellScale.x * 2), 1 - cs * cellScale.x - cellScale.x * 2);
        const centerY = Math.min(Math.max(Mouse.coords.y, -1 + cs * cellScale.y + cellScale.y * 2), 1 - cs * cellScale.y - cellScale.y * 2);
        const u = (this.mouse.material as THREE.RawShaderMaterial).uniforms;
        u.force.value.set(forceX, forceY); u.center.value.set(centerX, centerY); u.scale.value.set(cs, cs);
        super.update();
      }
    }
